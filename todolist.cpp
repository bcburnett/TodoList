...
void TodoList::keyboardButtonPressed() {
  ...
  if ( button && !(button->text() == "RESET")  ) {
    ...
    if(TodoList::mstate.gameover) enabledisableKeyboard(false);
...
void TodoList::on_RESET_clicked() {
  ...
  TodoList::mstate = parsephrase(getcatandphrase(clearstate(TodoList::mstate)));
...
state TodoList::parsephrase(state mstate) {
...
state TodoList::getcatandphrase(state mstate) {
  QSqlQuery query;
